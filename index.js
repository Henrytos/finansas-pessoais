...
function renderTrasaction(result) {
  ...
  title.classList = result.value > 0 ? "positeve-title" : "negative-title";
  title.innerText = result.transaction;

  const money = document.createElement("p");
  const valueTrasaction = result.value;
  money.innerText = `RS$ ${formatarNumero(valueTrasaction)}`;
...
async function excluitDELETE() {
  ...
  const articles = await fetch("http://localhost:3000/finance").then((res) =>
    res.json()
  );
...
function addMoney(money) {
  ...
  saldo.innerText = ` RS$ ${formatarNumero(moneyResult)}`;
